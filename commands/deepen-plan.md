---
description: 使用并行研究代理增强计划，为每个部分添加深度、最佳实践和实现细节
argument-hint: "[计划ID 或 文件路径] (如: P001 或 .factory/docs/plans/xxx.md)"
---
...
1. **如果为空** → 列出可用计划供选择：
   ```bash
   ls -la .factory/docs/plans/
   ```
   然后询问用户: "请选择要深化的计划，可以输入 ID（如 `P001`）或完整路径。"

2. **如果匹配 ID 格式** (如 `P001`, `P002`, `P003`)：
   - 在 `.factory/docs/plans/` 目录中查找以该 ID 开头的文件
   ```bash
   ls .factory/docs/plans/ | grep -i "^P001"
   ```
   - 如果找到唯一匹配 → 使用该文件路径
   - 如果找不到 → 提示 "未找到 ID 为 [ID] 的计划文件"
   - 如果有多个匹配（理论上不应该）→ 列出并让用户选择

3. **如果是文件路径** → 直接使用该路径

**Step 2: 验证文件存在**

确认解析后的文件路径存在，如果不存在则提示用户。

**最终结果:** 将解析后的完整文件路径存储为 `plan_path` 供后续步骤使用。
